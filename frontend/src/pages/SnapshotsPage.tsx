    ...
    <div className="flex flex-col h-full">
      <div className="flex items-center justify-between px-6 py-4 border-b border-gray-800">
        <h2 className="text-lg font-semibold text-gray-100 flex items-center gap-2">
          <Camera className="w-5 h-5 text-whale-400" /> 快照管理
        </h2>
        <button onClick={fetchSnapshots} className="btn-ghost text-sm flex items-center gap-1">
      ...
      <div className="flex-1 overflow-y-auto px-6 py-4">
        ...
        {snapshots.length === 0 ? (
          <div className="flex flex-col items-center justify-center py-16 text-gray-500">
            <Camera className="w-12 h-12 mb-3 text-gray-700" />
            <p className="text-sm">暂无快照</p>
            <p className="text-xs text-gray-600 mt-1">使用 Agent/YOLO 模式时会自动创建快照</p>
          </div>
        ) : (
          <div className="space-y-2">
            {snapshots.map((snap) => (
              <div key={snap.id as string} className="card">
                <div className="flex items-center justify-between">
                  <div>
                    <div className="flex items-center gap-2 mb-1">
                      <Camera className="w-4 h-4 text-gray-500" />
                      <span className="text-sm font-mono text-gray-200">{snap.id as string}</span>
                    </div>
